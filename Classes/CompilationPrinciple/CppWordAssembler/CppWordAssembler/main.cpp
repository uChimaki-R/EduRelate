	...
	for (int line_index = 0; line_index < size; line_index++) {
		...
		for (int i = 0; i < lines[line_index].size(); i++) {
			...
			if (is_split(lines[line_index][i])) {
				...
				output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t分界符号\n";
				now_token.clear();
			}
				...
				if (lines[line_index][i] == '<' && i + 1 < lines[line_index].size() && is_word(lines[line_index][i + 1])) {
					...
					if (is_identifier(name)) {
						// 定义的变量标识符
						output_string += "row[" + to_string(temp_line_index) + "]\t" + name + "\t标识符\n";
						...
						// 头文件直接输出
						name = "<" + name + ">";
						output_string += "row[" + to_string(temp_line_index) + "]\t" + name + "\t特殊符号\n";
						// 此时i在后面一个位置，即'>'，需要跳过
					}
				}
				// 如果是除号'/'也有两种特殊情况(注解"//"或"/*")
					...
					if (lines[line_index][i + 1] == '/') {
						...
						now_token = "//" + now_token;
						output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t注解\n";
						now_token.clear();
					}
					else {
						// "/*"
					...
					output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t运算符号\n";
				...
				if (is_keyword(now_token)) {
					// 关键字
					output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t关键字\n";
					now_token.clear(); // 清空token
				}
				else if (is_identifier(now_token)) {
					// 定义的变量标识符
					output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t标识符\n";
					now_token.clear(); // 清空token
				}
				else {
					// 是新定义的变量标识符
				...
				if (now_token.find('.') != now_token.npos) {
					// 浮点数
					output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t浮点数\n";
					now_token.clear(); // 清空token
				}
				else {
					// 整数
					output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t整数\n";
					now_token.clear(); // 清空token
				}
			}
				...
				// 此时i停在了'"'上，后面会自动跳过
				now_token = '"' + now_token + '"';
				output_string += "row[" + to_string(temp_line_index) + "]\t" + now_token + "\t字符串\n";
				now_token.clear(); // 清空token
			}
		}
	}
